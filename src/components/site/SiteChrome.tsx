  ...
  return (
    <header className="sticky top-0 z-40 w-full">
      ...
      <div className="hidden w-full text-white sm:block" style={{ background: "#0f172a" }}>
        <div className="mx-auto flex h-9 max-w-[1280px] items-center gap-5 px-5 text-[12px] text-white/80">
        ...
        <div className="mx-auto flex h-16 max-w-[1280px] items-center gap-4 px-5">
          <Link href={`/site/${config.slug}`} className="flex items-center gap-2.5">
            {config.logoUrl
              ...
              : <span className="text-[18px] font-extrabold tracking-tight text-[#0f172a]">{config.dealershipName}</span>}
          </Link>
          <nav className="ml-auto hidden items-center gap-7 lg:flex">
            {items.map((it) => {
              const active = pathname === it.href;
              return <Link key={it.href} href={it.href} className="text-[14px] font-medium transition" style={{ color: active ? accent : "#334155" }}>{it.label}</Link>;
            })}
          </nav>
          ...
          <button onClick={() => setOpen((v) => !v)} className="ml-auto grid h-10 w-10 place-items-center rounded-lg text-[#334155] lg:hidden">{open ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}</button>
        </div>
        {open && (
          <div className="border-t border-black/8 bg-white lg:hidden">
            <div className="mx-auto max-w-[1280px] px-5 py-2">
              {items.map((it) => <Link key={it.href} href={it.href} onClick={() => setOpen(false)} className="block py-2.5 text-[15px] font-medium text-[#334155]">{it.label}</Link>)}
  ...
  return (
    <footer className="w-full text-white" style={{ background: "#0f172a" }}>
      ...
      <div className="border-t border-white/10">
        <div className="mx-auto flex max-w-[1280px] flex-wrap items-center justify-between gap-2 px-5 py-4 text-[12px] text-white/50">
