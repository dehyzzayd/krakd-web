  ...
  return (
    <>
      ...
      <section className={`mx-auto ${C} px-5 py-4`}>
        ...
        {featured.length === 0 ? <Empty /> : <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">{featured.map((v) => <VehicleCard key={v.id} vertical={config.vertical} slug={config.slug} accent={accent} v={v} card={config.cardOptions} variant="soft" preview={preview} />)}</div>}
      </section>
  ...
  return (
    <>
      ...
      {show("shopByType") && makes.length > 0 && <section className={`mx-auto ${C} px-5 py-14`}>
        <h2 className="mb-5 font-display text-[30px] font-bold uppercase tracking-tight text-[#0a0a0a]">{config.shopHeading || "Shop by make"}</h2>
      ...
      <section className="pb-6">
        ...
        {featured.length === 0 ? <div className={`mx-auto ${C} px-6`}><Empty /></div> : (
          <div className="hscroll flex gap-6 overflow-x-auto px-6 pb-4" style={{ scrollSnapType: "x mandatory" }}>
            {featured.map((v) => <div key={v.id} className="w-[320px] shrink-0 sm:w-[380px]" style={{ scrollSnapAlign: "start" }}><VehicleCard vertical={config.vertical} slug={config.slug} accent={accent} v={v} card={config.cardOptions} variant="editorial" preview={preview} /></div>)}
          </div>
        )}
      </section>
  ...
  return (
    <>
      ...
      <section className={`mx-auto ${C} px-5 py-14`}>
        ...
        {featured.length === 0 ? <Empty /> : <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">{featured.map((v) => <VehicleCard key={v.id} vertical={config.vertical} slug={config.slug} accent={accent} v={v} card={config.cardOptions} variant="sharp" preview={preview} />)}</div>}
      ...
      {show("shopByType") && makes.length > 0 && <section className="w-full py-14" style={{ background: char }}><div className={`mx-auto ${C} px-5`}><h2 className="mb-5 font-display text-[28px] font-bold uppercase italic tracking-tight text-white">{config.shopHeading || "By make"}</h2><div className="flex flex-wrap gap-2.5">{makes.map((m) => <Link key={m} href={link(`/site/${config.slug}/inventory?make=${encodeURIComponent(m)}`)} className="border border-white/25 px-5 py-2.5 font-display text-[14px] font-semibold uppercase italic text-white transition hover:bg-white hover:text-[#141416]">{m}</Link>)}</div></div></section>}
  ...
  return (
    <div className="bg-white">
      ...
      <section className={`mx-auto ${C} ${rule} mt-14 px-6 py-14`}>
        ...
        {featured.length === 0 ? <Empty /> : <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">{featured.map((v) => <VehicleCard key={v.id} vertical={config.vertical} slug={config.slug} accent={accent} v={v} card={config.cardOptions} variant="sharp" preview={preview} />)}</div>}
      </section>
      {show("shopByType") && makes.length > 0 && <section className={`mx-auto ${C} ${rule} px-6 py-14`}><div className="flex items-baseline gap-4"><span className="text-[12px] tracking-[0.2em] text-[#999]">02</span><h2 className="text-[24px] font-medium tracking-tight text-[#111]">{config.shopHeading || "Browse"}</h2></div><div className="mt-6 flex flex-wrap gap-x-6 gap-y-3">{makes.map((m) => <Link key={m} href={link(`/site/${config.slug}/inventory?make=${encodeURIComponent(m)}`)} className="text-[15px] font-medium text-[#555] transition hover:text-[#111]">{m}</Link>)}</div></section>}
    ...
    <div style={{ background: navy }}>
      ...
      <section className="w-full bg-[#f3f3f7] py-16">
        <div className={`mx-auto ${C} px-5`}>
          ...
          {featured.length === 0 ? <div className="mt-8"><Empty /></div> : <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">{featured.map((v) => <VehicleCard key={v.id} vertical={config.vertical} slug={config.slug} accent={purple} v={v} card={config.cardOptions} variant="soft" preview={preview} />)}</div>}
  ...
  return (
    <>
      ...
      {show("shopByType") && cats.length > 0 && (
        <section className={`mx-auto ${C} px-5 py-14`}>
          <div className="mb-6 flex items-end justify-between"><h2 className="font-display text-[32px] font-bold uppercase italic tracking-tight text-[#0c0e10] sm:text-[40px]">{config.shopHeading || "Explore the lineup"}</h2></div>
