...
const createSchema = z.object({
  ...
  reconCents: z.coerce.number().int().min(0).optional(),
  packCents: z.coerce.number().int().min(0).optional(),
...
export const POST = route(async (req: NextRequest) => {
  ...
  const v = await prisma.vehicle.create({
    data: {
      ...
      stockNumber,
      year: d.year ?? null, make: d.make ?? null, model: d.model ?? null, trim: d.trim, bodyType: d.bodyType, category: d.category ?? null,
      ...
      ...(d.reconCents !== undefined ? { reconCents: d.reconCents } : {}), ...(d.packCents !== undefined ? { packCents: d.packCents } : {}),
      status: d.status, exteriorColor: d.exteriorColor, interiorColor: d.interiorColor, drivetrain: d.drivetrain, fuel: d.fuel, transmission: d.transmission, engine: d.engine,
