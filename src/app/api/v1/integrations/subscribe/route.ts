...
import { effectiveSub, isSubActive, startSub, cancelSub, reactivateSub, type IntegrationSub } from "@/lib/server/billing";
...
/* POST /api/v1/integrations/subscribe → paid-integration subscription lifecycle.
   Beta: activates free (no charge). At launch, `start` becomes a Stripe Checkout
   session and activation happens on the webhook — the state model here is unchanged. */
export const PUT = route(async (req: NextRequest) => {
  ...
  if (parsed.data.action === "start") {
    if (!isSubActive(sub)) { cur.subscription = startSub(def.priceCents); cur.enabled = true; }
