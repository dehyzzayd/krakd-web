...
import { Square, Heading, Type, AlignLeft, Image as ImageIcon, MousePointerClick, MoveVertical, Minus, Columns, Trash2, Copy, ChevronUp, ChevronDown, ArrowLeft, Loader2, Rocket, Monitor, Smartphone, GripVertical } from "lucide-react";

const ICON: Record<string, typeof Square> = { Square, Heading, Type, AlignLeft, Image: ImageIcon, MousePointerClick, MoveVertical, Minus, Columns };
...
export default function BuildPage() {
  ...
  const editProp = useCallback((id: string, patch: Record<string, unknown>) => setNodes((ns) => updateNode(ns, id, patch)), []);
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="w-[172px] shrink-0 overflow-y-auto border-r border-n200 bg-white p-3">
          ...
          <p className="mt-3 text-[10.5px] leading-relaxed text-n400">Drag the grip to reorder. Click text to edit it inline.</p>
        </aside>
            ...
            <div className="space-y-4">
              ...
              {selectedNode.type === "columns" && (
                <div className="flex gap-2">
                  <button onClick={() => setNodes((ns) => insertNode(ns, selectedNode.id, newNode("column")))} className="flex-1 rounded-md border border-n200 py-1.5 text-[12px] font-semibold text-n700 hover:bg-n50">+ Column</button>
                  <button onClick={() => setNodes((ns) => { const kids = findNode(ns, selectedNode.id)?.children ?? []; return kids.length > 1 ? removeNode(ns, kids[kids.length - 1].id) : ns; })} className="flex-1 rounded-md border border-n200 py-1.5 text-[12px] font-semibold text-n700 hover:bg-n50">− Column</button>
                </div>
              )}
