...
function AssignedPill({ value }: { value: string | null }) {
  return value
    ? <span className="inline-flex items-center gap-1 rounded-full border border-[#e4e7ec] px-2 py-1 text-[12px] text-n700"><UserIcon className="h-3 w-3" />{value === "AI" ? "Krakd AI" : value}</span>
...
function ActionsMenu({ r }: { r: Row }) {
  ...
  const items: { icon: React.ComponentType<{ className?: string }>; label: string; href?: string; primary?: boolean }[] = [
    ...
    { icon: Phone, label: "Call", href: `tel:${r.phone}` },
    { icon: MessageSquare, label: "Send SMS" },
    { icon: Mail, label: "Email", href: `mailto:${r.email}` },
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full z-30 mt-1 w-44 rounded-lg border border-[#e4e7ec] bg-white py-1 sh-raised">
      ...
      <div className="w-full px-6 py-5">
        ...
        <div className="pt-5">
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
            ...
            <button className="flex h-10 items-center gap-2 rounded-md border border-[#e4e7ec] bg-white px-4 text-[13px] font-medium text-n700 transition hover:bg-n50"><Download className="h-4 w-4" />Export</button>
          </div>
        ...
        {selected.size > 0 && (
          <div className="mt-4 flex items-center justify-between gap-3 rounded-xl border border-brand bg-brand-soft/50 px-4 py-2.5">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {["Assign", "Change status", "Send SMS"].map((a) => <button key={a} className="rounded-md border border-[#e4e7ec] bg-white px-3 py-1.5 text-[12px] font-semibold text-n700 hover:bg-n50">{a}</button>)}
            ...
            {view === "pipeline" ? (
              <div className="grid grid-cols-1 gap-3 p-4 md:grid-cols-3 lg:grid-cols-6">
                {STAGES.map((st) => { const items = filtered.filter((r) => r.stage === st.id); return (
                  <div key={st.id} className="flex min-h-[300px] flex-col rounded-2xl border border-n200 bg-n50">
                    <div className="flex items-center justify-between border-b border-[#e4e7ec] px-3 py-2.5"><span className="text-[11px] font-bold uppercase tracking-wide text-n700">{st.label}</span><span className="grid h-5 min-w-5 place-items-center rounded-full border border-[#e4e7ec] bg-white px-1.5 text-[11px] font-semibold text-n600">{items.length}</span></div>
                    <div className="flex flex-1 flex-col gap-2 p-2">{items.map((r) => (
                      <button key={r.id} onClick={() => router.push(`/dashboard/leads/${r.id}`)} className="rounded-lg border border-[#e4e7ec] bg-white p-2.5 text-left transition hover:border-brand/30 hover:shadow-sm">
                        ...
                        <div className="mt-1.5 flex items-center justify-between"><span className="rounded border border-[#e4e7ec] bg-white px-1.5 py-0.5 text-[10px] text-n600">{r.source}</span><span className="text-[10px] text-n400">{r.last}</span></div>
                      </button>
              ...
              <div className="w-full">
                <table className="w-full text-[13px]">
                  <thead className="bg-n50/60"><tr className="border-b border-[#e4e7ec] text-[11px] font-bold uppercase tracking-wide text-n500">
                    ...
                    <th className="px-2 text-left font-bold">Status</th><th className="px-2 text-left font-bold">Assigned</th><th className="px-2 text-left font-bold">Last added</th>
                    <th className="w-10 px-2"></th>
                  </tr></thead>
                  <tbody>
                    {filtered.length === 0 && <tr><td colSpan={8} className="p-8 text-center text-n400">No leads match the current filters.</td></tr>}
                    {filtered.map((r) => (
                      <tr key={r.id} onClick={() => router.push(`/dashboard/leads/${r.id}`)} className="cursor-pointer border-b border-n100 transition last:border-0 hover:bg-n50">
                        ...
                        <td className="p-2"><span className="inline-flex items-center rounded border border-[#e4e7ec] bg-white px-2 py-0.5 text-[11px] font-medium text-n600">{r.source}</span></td>
                        ...
                        <td className="tnum p-2 text-[12.5px] text-n500">{r.last} ago</td>
          ...
          {view === "list" && (
            <div className="mt-4 flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
              ...
              <nav className="flex items-center gap-1">
                ...
                <button className="grid size-9 place-items-center rounded-md border border-[#e4e7ec] bg-white text-[13px] font-medium shadow-sm">1</button>
